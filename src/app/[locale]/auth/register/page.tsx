        ...
        <TiltCard tiltScale={0.3}>
          <div className="glass-premium w-full p-8 md:p-10 rounded-[2.5rem] relative overflow-hidden border border-[var(--color-premium-rose)]/20 shadow-[0_0_50px_rgba(0,0,0,0.5)]">
            ...
            {refParam && checkingCode && (
              <div className="bg-white/[0.03] border border-white/10 p-3 rounded-xl mb-6 text-sm text-white/50 text-center">
                Vérification du code de parrainage...
              </div>
            )}
            {refParam && codeValid === true && (
              <div className="bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 p-3 rounded-xl mb-6 text-sm font-medium text-center">
                ✅ Code de parrainage <strong>{refParam}</strong> valide ! Vous serez lié à votre parrain.
              </div>
            )}
            {refParam && codeValid === false && (
              <div className="bg-amber-500/10 border border-amber-500/30 text-amber-400 p-3 rounded-xl mb-6 text-sm text-center">
                ⚠️ Code de parrainage "{refParam}" non reconnu. L'inscription est toujours possible.
              </div>
            )}
            ...
            <form onSubmit={handleSubmit} className="space-y-6 relative z-10">
              ...
              <div className="group">
                <label className="block text-sm font-bold text-gray-300 mb-2 group-focus-within:text-[var(--color-premium-rose)] transition-colors">
                  Code de parrainage <span className="text-[var(--color-premium-gray)]/40 font-normal">(optionnel)</span>
                </label>
                <input 
                  type="text"
                  value={referralCode}
                  ...
                  className="w-full bg-black/40 border border-white/10 rounded-2xl px-5 py-4 focus:outline-none focus:border-[var(--color-premium-rose)] focus:ring-1 focus:ring-[var(--color-premium-rose)]/50 transition-all text-white shadow-inner uppercase tracking-widest placeholder:normal-case placeholder:tracking-normal"
                  placeholder="EMB-XXXXXX"
                  maxLength={10}
                />
                <p className="text-xs text-[var(--color-premium-gray)]/50 mt-2 font-medium">
